...
) => toFixed((Math.atan2(p2.y - p1.y, p2.x - p1.x) * 180) / Math.PI);
...
export const intersectionRectangles = (rect1: Rectangle, rect2: Rectangle) => {
  let xColision = intersectionRanges(
    { p1: rect1.x, p2: rect1.x2 },
    { p1: rect2.x, p2: rect2.x2 }
  );
  let yColision = intersectionRanges(
    { p1: rect1.y, p2: rect1.y2 },
    { p1: rect2.y, p2: rect2.y2 }
  );
