...
import { HandsOptionsEvent } from './hands/domain/HandsEventOptions';
/**
 ...
 */
export const setEvents = (obj: any, events: string[], element: HTMLCanvasElement | HTMLElement | Document) => {
    for (const event of events) {
        ...
        if (hasEventMethod(obj, event)) {
            element.addEventListener(event, (e) => {
                if (!obj[method]) {
                    console.warn(`Event method '${method}' is not support in GameController`);
                } else {
                    obj[method]?.call(obj, e);
                }
            });
        }
    }
...
export const Controller = (props?: ControllerProps) => {
    return function _Controller<T extends { new (...args: any[]): any }>(constructor_: T) {
        return class extends injectable()(constructor_) {
            ...
            constructor(...args: any[]) {
                ...
                setEvents(_this, Object.values(HandsOptionsEvent), canvas);
            }
        };
    };
};
