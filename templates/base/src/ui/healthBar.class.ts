import { DOMContext, Drawable } from 'streetzero';
import { Health } from 'streetzero';

export class HealthBar extends Drawable {
    private _health;

    constructor(x: number, y: number, width: number, height: number, health: Health) {
        super(x, y, width, height);
        this._health = health;
    }
    render({ canvas, context }: DOMContext, background = 'grey', color = 'green') {
        ...
        context.fillRect(this.x + border, this.y + border, widthLife, this.height);
