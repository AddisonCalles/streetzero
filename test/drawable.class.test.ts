...
beforeAll(() => {});
test('setCanvas', () => {
  const draw = new Drawable(1, 2, 30, 33);
  expect(draw.canvas).toBe(canvas);
  expect(draw.context).toBe(context2D);
});
...
test('set offset', () => {
  const draw = new Drawable(5, 5, 30, 30);
...
test('centerOffset()', () => {
  const draw = new Drawable(5, 10, 30, 40);
  draw.centerOffset();
...
test('Leyer render', () => {
  ...
  wall.rect(0, 0, 30, -20);
  draw.setLeyers([new LayerPath(wall, 'red', new Kinematic(5, 10, 30, 40))]);
